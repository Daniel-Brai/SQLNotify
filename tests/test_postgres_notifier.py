...
class TestPostgreSQLNotifier:
    ...
    async def test_notifier_async_with_filters(
        ...
        async def on_filtered_user(event: ChangeEvent):
    ...
    async def test_notifier_async_multiple_subscribers_same_event(
        ...
        events_handler1: list[ChangeEvent] = []
        events_handler2: list[ChangeEvent] = []

        @notifier.subscribe(User, Operation.INSERT)
        async def handler1(event: ChangeEvent):
            events_handler1.append(event)

        @notifier.subscribe(User, Operation.INSERT)
        async def handler2(event: ChangeEvent):
            events_handler2.append(event)

        await notifier.astart()

        user = User(email="multi@example.com", name="Multi Sub User")
        async_session.add(user)
        await async_session.commit()
        ...
        assert events_handler1[0]["id"] == events_handler2[0]["id"]
    ...
    async def test_notifier_cleanup_on_start(
        ...
        notifier2.watch(User, Operation.INSERT, extra_columns=["email", "name"])

        await notifier2.astart()
        await notifier2.astop()

        await notifier2.acleanup()

    @pytest.mark.asyncio
    async def test_notifier_sync_mode(
        self,
        sync_engine,
        sync_session,
        cleanup_tables,
        ...
        notifier = Notifier(db_engine=sync_engine, cleanup_on_start=True)
        notifier.watch(User, Operation.INSERT, extra_columns=["email"])

        received_events: list[ChangeEvent] = []

        @notifier.subscribe(User, Operation.INSERT)
        def on_user_insert(event: ChangeEvent):
            received_events.append(event)

        notifier.start()

        user = User(email="sync@example.com", name="Sync User")
